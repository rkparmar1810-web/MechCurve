...
import { blogPostsBySlug } from '../data/blogContent';
...
export type SeoDefinition = {
	...
	robots: string;
	imageUrl: string;
	/** og:type — 'article' for blog posts so they preview as a story card. */
...
/**
 * Share card generated by scripts/generate-blog-og-images.mjs. JPEG at 1200x630
 * — WhatsApp will not render a WebP preview.
 */
export function blogOgImageUrl(slug: string) {
	return `${SITE_URL}/blog_og/${slug}.jpg`;
}
...
export function getSeoForPath(pathname: string): SeoDefinition {
	...
	if (blogMatch) {
		const post = blogPostsBySlug[blogMatch[1]];
		if (post) {
			return {
				title: `${post.title} | MechCurve Blog`,
				description: post.excerpt,
				canonicalUrl: `${SITE_URL}${path}`,
				robots: 'index, follow',
				imageUrl: blogOgImageUrl(post.slug),
				ogType: 'article',
			};
		}
	}
