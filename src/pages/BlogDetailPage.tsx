	...
	switch (block.type) {
		...
		case 'list':
			return (
				<motion.ul
					{...REVEAL}
					className="mt-5 grid max-w-[70ch] gap-2.5"
				>
...
export default function BlogDetailPage() {
	...
	// Parallax on the full-bleed cover: the photo drifts and scales slightly as
	// the page scrolls past it, so the banner has depth instead of sliding away
	// as a flat block.
	...
	useSeo({
		title: post ? `${post.title} | MechCurve Blog` : 'Article Not Found | MechCurve',
		description: post
			? post.excerpt
			...
			<section className="site-light-content px-5 pb-16 pt-32 sm:px-8 md:px-12">
				<div className="mx-auto max-w-3xl rounded-3xl border border-slate-200 bg-white p-10 text-center shadow-[0_24px_60px_rgba(15,23,42,0.08)]">
					...
					<p className="mt-3 text-slate-600">
						This article is unavailable. You can return to the blog and browse the
						other posts.
					</p>
					<Link
						to="/blog"
	...
	return (
		<div className="site-light-content">
			{/* ── Hero: title band, then a full-bleed cover ── */}
			<section className="relative overflow-hidden pt-28 lg:pt-32">
				...
				<div className="mx-auto max-w-[1440px] px-5 pb-10 sm:px-8 md:px-12">
					<motion.nav
					...
					>
						...
						<Link to="/blog" className="transition-colors hover:text-amber-700">
							Blog
						</Link>
						<LuChevronRight size={14} className="text-slate-400" />
					...
					<motion.div
						className="mt-6 inline-flex items-center gap-2 rounded-full border border-amber-300 bg-amber-50 px-3 py-1 text-[10px] font-bold uppercase tracking-[0.16em] text-amber-700"
					...
					<motion.h1
						// No max-width: the headline runs the full container width. `pretty`
					// (not `balance`, which would even the lines out and pull the first
					// one short of the edge) only fixes a lonely last word.
					// leading-[1.25] keeps descenders clear of the line below at this size.
					className="mt-5 text-[1.75rem] font-extrabold leading-[1.25] tracking-[-0.01em] text-slate-900 [text-wrap:pretty] sm:text-[2.1rem] md:text-[2.5rem] lg:text-[3.1rem] xl:text-[3.5rem]"
						initial={{ opacity: 0, y: 18 }}
						animate={{ opacity: 1, y: 0 }}
					...
					<motion.div
						className="mt-5 flex flex-wrap items-center gap-x-5 gap-y-2 text-[13px] font-medium text-slate-500"
						initial={{ opacity: 0, y: 14 }}
						animate={{ opacity: 1, y: 0 }}
				...
				{/* Full-bleed banner — deliberately outside the padded container.
				    The outer div clips; the inner one is what actually parallaxes. */}
				<motion.div
					ref={coverRef}
				...
				>
					<motion.div
						style={
							reduceMotion ? undefined : { y: coverY, scale: coverScale }
						}
						className="will-change-transform"
					>
						<BlogCoverImage slug={post.slug} title={post.title} variant="full" />
					</motion.div>

					{/* Fades the photo into the white article background below it. */}
					<div
			...
			{/* ── Article + sidebar ── */}
			<section className="px-5 pb-20 pt-12 sm:px-8 md:px-12" aria-label="Article">
				<div className="mx-auto grid max-w-[1440px] grid-cols-1 gap-10 lg:grid-cols-[minmax(0,1fr)_320px] lg:gap-14">
					{/* Body */}
					<article>
						<p className="max-w-[70ch] border-l-4 border-amber-400 pl-5 text-lg font-medium leading-9 text-slate-600 md:text-xl">
							{post.excerpt}
						</p>
